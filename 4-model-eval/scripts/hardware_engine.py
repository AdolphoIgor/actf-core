...
def get_evaluation_setup(
...
) -> tuple[torch.device, torch.dtype, Any, Any]:
    """
    Resolves hardware devices and initializes a PyTorch Causal LM
    and Tokenizer for log-likelihood evaluation and token-level scoring.
    """
    if device_override:
    ...
    return device, dtype, model, tokenizer


def get_inference_engine(model_id: str) -> Any | tuple[Any, Any]:
    """
    Initializes high-throughput vLLM engine if an NVIDIA GPU is available;
