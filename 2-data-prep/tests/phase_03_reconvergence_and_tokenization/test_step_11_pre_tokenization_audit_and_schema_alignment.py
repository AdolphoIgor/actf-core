...
from scripts.phase_03_reconvergence_and_tokenization.step_11_pre_tokenization_audit_and_schema_alignment import PreTokenizationAuditor
...
def test_pre_tokenization_null_audit():
    ...
    input_table = pa.Table.from_pydict({
        "text": ["Valid text", "", None, "More valid text"],
        "branch_id": [0, 0, 1, 1]
    })
...
def test_pre_tokenization_policy_injection():
    ...
    input_table = pa.Table.from_pydict({
        "text": ["General prose.", "def code_block(): return True"],
        "branch_id": [0, 1]
    })
