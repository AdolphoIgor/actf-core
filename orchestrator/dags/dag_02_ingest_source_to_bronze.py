...
# Dual import resolution for local workspace and container runtimes
try:
    from dags.scripts.quality_gate_1 import execute_bronze_quality_gate
except ImportError:
...
Extracts raw structured logs from PostgreSQL (via PySpark) and unstructured text documents
from the landing zone (via Ray Data) into the Medallion Bronze Parquet store.
"""
...
def check_postgres_delta(state_key):
    ...
    print(
        f"Pre-processing Postgres Check: Found {count} new records since high-watermark '{last_ts}'."
    )

    if count == 0:
        raise AirflowSkipException(
            "Zero new database records found. Skipping Spark ingestion execution."
        )
...
def check_files_delta(state_key):
    ...
    print(
        f"Pre-processing File Check: Found {new_files_count} new documents since high-watermark '{last_sync}'."
    )

    if new_files_count == 0:
        raise AirflowSkipException(
            "Zero new files found in landing zone. Skipping Ray job submission."
        )
...
def submit_ray_job(state_key):
    ...
    job_payload = {"entrypoint": f"python {script_path} --last_sync {last_sync}"}
    ...
    if status != "SUCCEEDED":
        raise Exception(
            f"Ray Job {job_id} failed with status: {status}. Review logs above for details."
        )
...
with DAG(
    ...
    quality_gate_validation = PythonOperator(
        ...
        op_kwargs={"bucket_name": "company-ai-datalake"},
    )
